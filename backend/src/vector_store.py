...
def add_chunks(chunks: List[Dict]) -> None:
    ...
    metadatas = [{
        ...
        "location_label": c.get("location_label", f"Page {c['page_number']}"),
...
def list_active_documents() -> List[Dict]:
    """
    Return a de-duplicated list of {doc_id, filename, file_type} for
    everything currently stored in the vector database.
    """
    collection = get_collection()
    ...
    return [
        {"doc_id": doc_id, "filename": info["filename"], "file_type": info["file_type"]}
...
def query(query_text: str, doc_ids: Optional[List[str]] = None,
    ...
    for i in range(len(ids)):
        hits.append({
            ...
            "location_label": metas[i].get("location_label", f"Page {metas[i].get('page_number')}"),
            "file_type": metas[i].get("file_type", "pdf"),
...
def get_all_chunks_for_doc(doc_id: str) -> List[Dict]:
    ...
    return get_chunks_for_doc_ids([doc_id])


def get_chunks_for_doc_ids(doc_ids: List[str]) -> List[Dict]:
    """
    Fetch every chunk belonging to any of the given doc_ids. Used both for
    whole-paper summarization and to build the BM25 lexical corpus for the
    currently active/selected papers (BM25 needs the full candidate corpus,
    not just the top semantic matches, to compute meaningful term statistics).
    """
    if not doc_ids:
        return []
    collection = get_collection()
    result = collection.get(where={"doc_id": {"$in": doc_ids}}, include=["documents", "metadatas"])
    chunks = []
    for i in range(len(result.get("ids", []))):
        ...
        chunks.append({
            ...
            "location_label": meta.get("location_label", f"Page {meta.get('page_number')}"),
            "file_type": meta.get("file_type", "pdf"),
